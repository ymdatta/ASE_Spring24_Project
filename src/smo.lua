...
smo: semi-supervosed multi-objective explanation
...
-- Update NB
function l.nb(nb1,xs)
  xs = xs.cells and xs or ROW(xs)
  if    nb1.all
  then  want = k.nbHas(nb1, xs)
        l.nbTest(nb1, xs,want)
        l.nbTrain(k, xs, want)
  else  nb1.all = DATA{xs} end end

-- Ensure we have a place to store data for this klass. 
function l.nbHas(nb1, row)
  want = row.has[nb1.all.cols.klass.at]
  if not nb1.klasses[want] then
    nb1.h = nb1.h + 1
    nb1.klasses[want] = l.clone(nb1, all) end
  return nb1.klasses[want] end

-- If we've waited enough, try classifying something.
function l.nbTest(nb1 ,row,want,       k)
  if   nb1.wait > 1  
  then nb1.wait = nb1.wait - 1
  else got = l.likesMost(row.cells, nb1.datas, #nb1.all.rows, nb1.h)
       l.abcd(nb1.abcd, want, go) end end

-- Update our distributions.
function l.nbTrain(nb1, row,want)
  l.data(nb1.datas[want], xs)
  l.data(nb1.all,         xs) end
